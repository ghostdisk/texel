...
@group(0) @binding(1) var generated: texture_2d<f32>;
@group(0) @binding(2) var selection: texture_2d<f32>;
...
@compute @workgroup_size(8, 8) fn main(@builtin(global_invocation_id) id: vec3u) {
  ...
  let before = textureLoad(original, id.xy, 0);
  let after = textureSampleLevel(generated, imageSampler, uv, 0);
  var coverage = 1.0;
  if (params.hasMask > 0.5) { coverage = clamp(textureLoad(selection, id.xy, 0).r, 0.0, 1.0); }
  textureStore(destination, id.xy, mix(before, after, coverage));
}