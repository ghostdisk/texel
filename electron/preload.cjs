...
contextBridge.exposeInMainWorld('desktop', {
  ...
  chooseImageExport: (format, name, key, documentToken) => ipcRenderer.invoke('image:choose-export', format, name, key, documentToken),
  chooseImageExportDirectory: (key, documentToken) => ipcRenderer.invoke('image:choose-export-directory', key, documentToken),
  writeImageExport: (token, bytes) => ipcRenderer.invoke('image:write-export', token, bytes),
  writeImageExportDirectory: (token, name, format, bytes) => ipcRenderer.invoke('image:write-export-directory', token, name, format, bytes),
