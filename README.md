...
TXL version 1 uses a small binary header, a JSON metadata chunk, and a binary chunk of lossless half-float source pixels. It retains canonical dimensions, layer/group structure, transforms, filters, masks, selections, grid and guides, and the generation lens. See [the format specification](docs/txl-format.md).
...
- **O** selects Ellipse; hold Shift while dragging for a circle. **L** draws a closed Freehand Lasso shape. **P** starts a Polygon Lasso: click vertices, then press Enter, double-click, or click the first point to fill; Backspace removes a vertex and Escape cancels. All three paint the current pixel layer or mask with opacity, erase, and active-selection clipping.
- **F** selects Fill. Tolerance compares source color and alpha; Contiguous fills the connected region, while disabling it fills all matching pixels. Both honor opacity, erase mode, and the active selection. Escape cancels a pending fill. GPU scanline spans use packed matching/visited bits and indirect dispatch; no pixels or queue counters are read back. A canvas-derived run bound ensures completion even for winding regions, with one undo entry after the mask is finished.
- **K** selects Clone Stamp and **H** selects Healing Brush. Alt-click inside the selected color image or text layer to set a raw source point, then paint on an editable RGBA pixel layer with Size, Hardness, Flow, and Aligned controls. Clone copies premultiplied source pixels from an immutable stroke snapshot. Healing transfers source detail over the destination's blurred local color; transparent destination neighborhoods fall back to the source. Both honor the active soft selection and erase mode. Retouching masks is intentionally unavailable.
- **T** selects Text. Click to add a text layer, or click within the selected text to edit it. The options bar controls font, size, bold/italic, color and alignment; Edit text contains multiline content and line height. Ctrl+Enter finishes editing. Text retains its settings and a Canvas-rendered GPU image, with undo, filters and transforms. Pixel tools, Cut, reframing and filter baking do not edit text; Layer via Copy makes a pixel copy of selected text.
- **V** selects Move / transform. Ctrl-click visible pixels to select their layer; transparent areas pass through to layers below. Drag the selected layer to move it, drag its eight bounds handles to resize, and drag the round handle to rotate. Shift constrains movement, corner resizing, or rotation; Alt bypasses snapping. Arrow keys nudge by one world pixel and Shift+Arrow by ten. Position, scale, rotation, alignment, distribution, 90° rotation, and flip controls appear in this tool. One layer aligns to the canvas; multiple layers align to their union, and distribution spaces centers while keeping the outer layers fixed.
...
- The **View** menu toggles the grid, guides, and snapping. Grid and guides opens compact controls for grid spacing and horizontal or vertical guide positions. Grid density adapts to zoom, while snapping compares canvas, visible grid and guide lines, and other visible layer edges and centers within a six-screen-pixel threshold.
...
The document has canonical pixel dimensions exposed as "width" and "height" on ImageDocument. These dimensions define the canvas rectangle and initial layer size; Crop can later change the rectangle without changing source pixels. Each image layer still has its own native resolution and affine transform into its parent. A 2000 × 2000 source stays at that resolution when transformed onto a 500 × 500 canvas. Groups rasterize their transformed children at a density chosen for the current view. Presentation clips to the canvas; layer pixels outside it remain available.
