...
struct Session {
    uint64_t id;
    TexelLocalAiHost host;
    std::atomic<bool> authenticated{false};

    Session(uint64_t session_id, TexelLocalAiHost callbacks) : id(session_id), host(callbacks) {}
};
...
static void send_json(const std::shared_ptr<Session>& session, const Json& value) {
    ...
    session->host.send_text(session->host.user, session->id, data.data(), data.size());
}
...
static void send_preview(Job& job, const sd_image_t& image, int step) {
    ...
    const auto data = encode_image(job, "preview", image, step);
    job.session->host.send_binary(job.session->host.user, job.session->id,
        reinterpret_cast<const uint8_t*>(data.data()), data.size());
}

class Backend {
    TexelLocalAiHost host_;
    ...
    void generate(const std::shared_ptr<Job>& job) {
        ...
        try {
            ...
            else job->session->host.send_binary(job->session->host.user, job->session->id,
                reinterpret_cast<const uint8_t*>(result.data()), result.size());
        } catch (const std::exception& error) { std::cerr << "Job completion: " << error.what() << '\n'; }
    }

public:
    Backend(TexelLocalAiHost host, fs::path root, fs::path config, std::string token) : host_(host), model_root_(std::move(root)), token_(std::move(token)) {
    ...
    void connect(uint64_t id) {
        ...
        sessions_[id] = std::make_shared<Session>(id, host_);
    }
    ...
    void message(uint64_t id, const std::string& data, bool binary) {
        std::shared_ptr<Session> session;
        {
            std::lock_guard<std::mutex> lock(sessions_mutex_);
            const auto found = sessions_.find(id);
        ...
        message(session, data, binary);
    }

    void write_log(const char* data) const {
        if (host_.log && data) host_.log(host_.user, data, std::strlen(data));
    }
        ...
        try {
            ...
            if (!session->authenticated) {
                if (binary || type != "hello" || request.value("token", std::string{}) != token_) {
                    if (session->host.close) session->host.close(session->host.user, session->id, 1008, "Authentication required");
                    return;
                }
...
static void copy_error(char* destination, size_t size, const char* message) {
    ...
    std::snprintf(destination, size, "%s", message ? message : "Unknown local AI backend error.");
}

static void* TEXEL_LOCAL_AI_CALL create_backend(
    const TexelLocalAiHost* host,
    const TexelLocalAiConfig* config,
    char* error,
    size_t error_size) {
    try {
        if (!host || host->abi_version != TEXEL_LOCAL_AI_ABI_VERSION || !host->send_text || !host->send_binary ||
            !config || config->abi_version != TEXEL_LOCAL_AI_ABI_VERSION || !config->model_root || !config->model_config) {
            throw std::runtime_error("Invalid local AI backend configuration.");
        }
        const char* token = std::getenv("IMGED_BACKEND_TOKEN");
        if (!token || std::strlen(token) < 32) {
            throw std::runtime_error("IMGED_BACKEND_TOKEN must contain at least 32 characters.");
        }
        if (const char* shader_error = texel_shader_archive_error()) {
            throw std::runtime_error(std::string("Cannot load Vulkan shaders: ") + shader_error);
        }
        auto backend = std::make_unique<Backend>(*host, config->model_root, config->model_config, token);
        sd_set_log_callback([](sd_log_level_t level, const char* text, void* data) {
            if (level >= SD_LOG_INFO) static_cast<Backend*>(data)->write_log(text);
        }, backend.get());
...
static void TEXEL_LOCAL_AI_CALL destroy_backend(void* backend) {
    sd_set_log_callback(nullptr, nullptr);
    delete static_cast<Backend*>(backend);
    texel_vk_release_instance();
}

static void TEXEL_LOCAL_AI_CALL connect_backend(void* backend, uint64_t session) {
    static_cast<Backend*>(backend)->connect(session);
}

static void TEXEL_LOCAL_AI_CALL disconnect_backend(void* backend, uint64_t session) {
    static_cast<Backend*>(backend)->disconnect(session);
}

static void TEXEL_LOCAL_AI_CALL message_backend(
    void* backend,
    uint64_t session,
    const uint8_t* data,
    size_t size,
    bool binary) {
    if (!backend || (!data && size)) return;
    static_cast<Backend*>(backend)->message(session,
        std::string(reinterpret_cast<const char*>(data), size), binary);
}

extern "C" TEXEL_LOCAL_AI_EXPORT const TexelLocalAiBackend* TEXEL_LOCAL_AI_CALL texel_local_ai_backend() {
    static const TexelLocalAiBackend api{
